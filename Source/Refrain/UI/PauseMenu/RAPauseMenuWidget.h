 ...
 * @note 메뉴의 기능을 담당하며 위젯 블루프린트 작성시 바인딩하여 작성하도록 합니다. 카운트인이 이후에 별도로 필요하다면 분리하세요.
 * @date 2026-07-01
 * @author Sejong
 */
UCLASS()
...
public:
	...
	/** @brief 카운트인을 시작하고 버튼 메뉴를 숨깁니다. */
	void StartCountIn();
	
	/** @brief 카운트인을 취소하고 다시 버튼 메뉴를 표시합니다. */
	void CancelCountIn();

	// --- Getter ---
	bool IsCountingIn() const { return bIsCountingIn; }
...
protected:
	...
	/** @brief 재개 전 카운트다운 숫자를 표시할 텍스트 (블루프린트에서 필수로 바인딩하지는 않으므로 Optional 처리) */
	UPROPERTY(meta = (BindWidgetOptional))
	TObjectPtr<class UTextBlock> CountInText;

	/** @brief 카운트인 진행 여부 */
	bool bIsCountingIn = false;
	
	/** @brief  */
	float CountInTime = 0.0f;
